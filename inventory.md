...
### Retrieve Inventory API [GET]

+ Response 200 (application/vnd.com.nsn.cumulocity.inventoryApi+json)
...
Read-only collection of managed objects containing a text value starting with the given text (placeholder «text»). Text value is any alphanumeric string starting with a latin letter (A-Z or a-z).
...
### POST â€“ Create a new ManagedObject
...
Username of user which owns this ManagedObject, if not filled automatically allocated when the object is created. Only user with role "ROLE\_INVENTORY\_ADMIN" can explicitly specify owner .
...
A collection of references to parent objects.

No

A managed object reference in the "parents" collection only contains "self" references. In "childDevices" and "childAssets", "id" and "name" are additionally included.
...
### PUT â€“ Update a Managed object

Request body: ManangedObject
 Response body: ManangedObject (when accept header is not provided, empty response body is returned)
 Required : role ROLE\_INVENTORY\_ADMIN or user has to be owner of ManagedObject
...
### POST â€“ add a managed object reference to the collection
...
Required : role ROLE\_INVENTORY\_ADMIN or user has to be owner
